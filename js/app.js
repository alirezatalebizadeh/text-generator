...
let btnGenerator = $.querySelector('button')
...
function createText(event) {
  ...
  if (valueInput > 36 || isNaN(valueInput) || valueInput < 0 || valueInput === '' || valueInput == 0) {
    resElem.innerHTML = dummyText[randomTextIndex]
    alert('please enter a number between 1 and 36')
    // console.log(dummyText[randomTextIndex]);
  } else {
    let sliceTextArray = dummyText.slice(0, valueInput)

    let textWithOutPelem;
    let text = sliceTextArray.map(text => {
      textWithOutPelem += text
      return `<p>${text}</p>`
    }).join('')
    resElem.innerHTML = text
    copyText(textWithOutPelem)
    btnGenerator.className = 'btn btn-success'
  }
}


function changeBtn() {
  btnGenerator.className = 'btn btn-primary'
}
...
btnGenerator.addEventListener('click', createText)
